...
function ModalityBadge({ modality }: { modality: string }) {
  ...
  return (
    <span className={`px-2 py-0.5 rounded border text-xs font-semibold ${cfg[m] ?? "bg-gray-50 dark:bg-surface-raised text-gray-600 dark:text-gray-400 dark:text-gray-500 border-gray-200 dark:border-gray-700"}`}>
      {m || "—"}
    </span>
  );
}
...
export default function UploadPage() {
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      ...
      {/* ── How it works ─────────────────────────────────────────────────── */}
      <div className="bg-blue-50 dark:bg-blue-950 border border-blue-100 rounded-xl px-5 py-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="flex gap-3">
            <div className="shrink-0 w-7 h-7 rounded-full bg-white dark:bg-surface border border-blue-200 flex items-center justify-center text-xs font-bold text-blue-600 dark:text-blue-400">1</div>
            <div>
          ...
          <div className="flex gap-3">
            <div className="shrink-0 w-7 h-7 rounded-full bg-white dark:bg-surface border border-blue-200 flex items-center justify-center text-xs font-bold text-blue-600 dark:text-blue-400">2</div>
            <div>
              <div className="flex items-center gap-1.5 mb-1">
          ...
          <div className="flex gap-3">
            <div className="shrink-0 w-7 h-7 rounded-full bg-white dark:bg-surface border border-blue-200 flex items-center justify-center text-xs font-bold text-blue-600 dark:text-blue-400">3</div>
            <div>
              <div className="flex items-center gap-1.5 mb-1">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
          {[
            ...
            <div key={label}>
              ...
              <p className="font-mono font-semibold text-gray-900 dark:text-gray-100 bg-gray-50 dark:bg-surface-raised border border-gray-200 dark:border-gray-700 rounded px-2 py-1 text-xs">
                {value}
              </p>
            </div>
          ))}
      ...
      <div className="bg-white dark:bg-surface rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800">
          ...
          <button
            onClick={() => mutate()}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-600 dark:text-gray-400 dark:text-gray-500 hover:text-gray-900 dark:hover:text-gray-100 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-surface-raised transition-colors"
          >
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                {filtered.map((s) => {
                  ...
                  return (
                    <tr
                      key={s.orthanc_id}
                      className={`transition-colors ${inPlatform ? "bg-green-50/30" : "hover:bg-gray-50 dark:hover:bg-surface-raised"}`}
                    >
                      <td className="px-5 py-3">
                        <p className="font-medium text-gray-900 dark:text-gray-100">{formatPatientName(s.patient_name)}</p>
                      </td>
                      <td className="px-3 py-3 text-gray-500 dark:text-gray-400 dark:text-gray-500 font-mono text-xs">{s.patient_id || "—"}</td>
                      <td className="px-3 py-3 text-gray-600 dark:text-gray-400 dark:text-gray-500 whitespace-nowrap">{formatDate(s.study_date)}</td>
                      <td className="px-3 py-3 text-gray-600 dark:text-gray-400 dark:text-gray-500 max-w-[200px]">
                      ...
                      <td className="px-3 py-3 text-gray-600 dark:text-gray-400 dark:text-gray-500 text-center">{s.series_count}</td>
      ...
      <div className="bg-white dark:bg-surface rounded-xl border border-gray-200 dark:border-gray-700">
        <button
          ...
          className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-gray-50 dark:hover:bg-surface-raised transition-colors rounded-xl"
        >
          <div>
