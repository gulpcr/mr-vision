...
def load_image_encoder(config_image_encoder: Dict):
    if config_image_encoder["source"].lower() == "huggingface":
        ...
        # VENDORED EDIT (inference): from_name (no ImageNet download) — the Mammo-CLIP
        # checkpoint state_dict is loaded strict=False afterward, so base init is irrelevant.
        _image_encoder = EfficientNet.from_name("efficientnet-b2", num_classes=1)
        _image_encoder.out_dim = 1408
    elif (
            config_image_encoder["source"].lower() == "cnn" and
            config_image_encoder["name"].lower() == "tf_efficientnet_b5_ns-detect"
    ):
        # VENDORED EDIT (inference): from_name instead of from_pretrained — see note above.
        _image_encoder = EfficientNet.from_name("efficientnet-b5", num_classes=1)
