...
def load_pretrained_weights(
    ...
    # VENDORED EDIT (inference): when clip_chk_pt is None, skip seeding the backbone
    # from the Mammo-CLIP checkpoint. Used when the trained detector .pth (which already
    # contains the full fine-tuned backbone) is loaded strict=False right after — avoids
    # re-loading the 1.6 GB clip tar on every build.
    if clip_chk_pt is None and (model_type == "breast_clip_b5" or model_type == "breast_clip_b2"):
