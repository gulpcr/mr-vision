...
async def list_audit_logs(
    ...
    stmt = select(AuditLogRecord).where(AuditLogRecord.tenant_id == tenant_id).order_by(
        AuditLogRecord.timestamp.desc()
    )
    count_stmt = select(func.count()).select_from(AuditLogRecord).where(
...
async def verify_audit_chain(
    ...
    """Walk the audit_log hash chain and confirm no entry was altered,
    deleted, or reordered since the chain was introduced (migration 032).
    Platform-admin only — this spans every tenant's audit history.
    """
...
async def create_batch(
    ...
    await PgAuditRepository(session).save(AuditEntry(
        ...
        entity_id=batch["id"],
        actor=creator,
        details={"name": body["name"], "study_count": len(body["study_uids"])},
        tenant_id=tenant_id,
    ))
    return batch
...
async def get_prior_comparison(
    ...
    # Find most recent prior result for same patient + usecase. patient_id is
    # only unique within a tenant, so the tenant filter here is load-bearing —
    # without it, a patient_id collision across tenants would surface another
    # tenant's prior result as this patient's comparison baseline.
    prior_stmt = (
        select(ResultRecord)
...
async def reset_all_data(
    ...
    Clears:
      ...
      - MinIO artifacts for the affected studies
    ...
    Scoped to the CALLER's own tenant by default — this used to wipe every
    tenant's data unconditionally, which is a data-loss bug in a multi-tenant
    deployment. Pass ?all_tenants=true for the old global-wipe behavior,
    which now requires platform-admin (see require_platform_admin).
    ...
    if all_tenants and not getattr(request.state, "is_platform_admin", False):
        raise HTTPException(
            403, "Wiping all tenants requires platform admin access — omit all_tenants to reset your own tenant only."
        )

    scope_tenant_id = None if all_tenants else tenant_id
    ...
    # Study UIDs affected by this reset — needed to scope the MinIO cleanup
    # below to exactly these studies' artifacts, not the whole bucket.
    ...
    # ── 3. MinIO — remove artifacts for the affected studies only ───
    ...
    try:
        ...
        def _bulk_delete() -> tuple[int, int]:
            ...
            if scope_tenant_id:
                # Storage keys are "{study_instance_uid}/...", so scoping to a
                # tenant means listing per study prefix rather than the bucket root.
                objects = []
                for study_uid in affected_study_uids:
                    objects.extend(
