...
class LoginResponse(BaseModel):
    ...
    enabled, a short-lived mfa_token to be exchanged at /mfa/verify — never
    both, and never a real access_token without a code being verified first.
    """
...
def _mfa_locked_exception(e) -> HTTPException:
...
async def get_current_user(
    ...
    """The CURRENT authenticated user's own profile — used by the frontend to
    reflect live MFA/role state rather than the stale copy cached at login.
    """
    service = AuthService(session)
    user = await service.get_user_by_id(request.state.user_id)
    if not user:
...
async def login(
    ...
    """Authenticate user. Returns a real token, or — if the account has MFA
    enabled — an mfa_token to complete at POST /auth/mfa/verify.
    """
    service = AuthService(session)
    try:
...
async def verify_mfa(
    ...
    mfa_token from /login plus a TOTP/recovery code for a real access token.
    """
    from app.application.mfa_service import MfaLockedError
    ...
    if not result:
        raise HTTPException(status_code=401, detail="Invalid or expired MFA token/code")
    return LoginResponse(**result)
...
async def list_users(
    ...
    """List users, scoped to the caller's own tenant by default.

    Previously this called service.list_users() with NO filter at all —
    returning EVERY tenant's users to anyone holding the per-tenant
    "user.manage" permission (which require_permission auto-grants to any
    role=="admin"/"system" user). Viewing another tenant's users now
    requires is_platform_admin, matching the same cross-tenant boundary
    already enforced on tenant lifecycle / API-key / plan endpoints.
    """
    ...
    if tenant_id and tenant_id != caller_tenant_id and not getattr(request.state, "is_platform_admin", False):
        raise HTTPException(
            status_code=403,
            detail="Viewing another tenant's users requires platform admin access",
        )
...
async def update_platform_admin_status(
    ...
    Only an existing platform admin can call this — there is deliberately no
    self-bootstrap path (see AuthService.set_platform_admin); the very first
    platform admin must still be set directly against the DB.
    """
    service = AuthService(session)
...
async def update_platform_operator_status(
    ...
    """Grant or revoke platform-operator status (impersonation rights only).

    Only a platform admin can call this — an operator cannot grant itself or
    others operator rights.
    """
    service = AuthService(session)
    ...
    await audit_repo.save(
        AuditEntry(
            action=(
                AuditAction.PLATFORM_OPERATOR_GRANTED
                if body.is_platform_operator
                else AuditAction.PLATFORM_OPERATOR_REVOKED
            ),
            entity_type="user",
            entity_id=user_id,
            actor=actor,
...
async def start_impersonation(
    ...
    """Mint a short-lived (30 min) token that IS the target user — their own
    role/tenant/permissions, never elevated by the operator's own access.
    """
...
async def stop_impersonation(
    ...
    """End the CURRENT request's impersonation session — revokes this one
    token via the Redis blocklist (not a whole-user session kill; the
    operator's own real token is untouched and keeps working).
    """
...
async def enroll_mfa(
    ...
    """Start MFA enrollment for the CURRENT authenticated user — generates a
    new secret and QR code. totp_enabled stays False until /mfa/confirm
    verifies a real code from an authenticator app.
    """
...
async def confirm_mfa(
    ...
    """Confirms enrollment and returns recovery codes — shown once, in
    plaintext; only their hashes are stored.
    """
    from app.application.mfa_service import MfaLockedError
    ...
    audit_repo = PgAuditRepository(session)
    await audit_repo.save(AuditEntry(
...
async def disable_mfa(
    ...
    """Turn MFA off for the CURRENT authenticated user. Requires a currently
    valid TOTP or recovery code — the password alone is not enough.
    """
    from app.application.mfa_service import MfaLockedError
