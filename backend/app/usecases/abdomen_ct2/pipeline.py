...
VQVAE anomaly selector) and hands them straight to MedGemma — ``abdomen_ct2`` does
...
hook, using ``app.usecases.abdomen_ct2.report`` + ``prompts``). The rendered slices
...
class Pipeline(BasePipeline):
    ...
    def __init__(self) -> None:
        ...
        self._model_version = "abdomen_ct2_medgemma_v1.0.0"
    ...
    def preprocess(
        ...
        if primary is None:
            raise ValueError("No series found for abdomen_ct2 pipeline")
        ...
        logger.info(
            "abdomen_ct2_preprocess_complete",
    ...
    def infer(self, preprocessed: dict[str, Any], working_dir: str) -> dict[str, Any]:
        # The "inference" for abdomen_ct2 is the MedGemma two-pass scan/report, which
        ...
        logger.info("abdomen_ct2_infer_passthrough")
    ...
    def postprocess(self, inference_output: dict[str, Any], working_dir: str) -> dict[str, Any]:
        logger.info("abdomen_ct2_postprocess_start")
        ...
        logger.info(
            "abdomen_ct2_postprocess_complete",
    ...
    def _candidate_slices(self, arr: np.ndarray) -> list[int]:
        ...
        if len(kept) > cap:
            ...
            logger.info("abdomen_ct2_candidates_strided", available=len(kept), scanned=len(picked))
            return picked
        return kept
    ...
    def _render_png(self, slice_2d: np.ndarray, out_path: str, level: float, width: float) -> bool:
        ...
        try:
            ...
            logger.warning("abdomen_ct2_render_failed", error=str(exc))
            return False
