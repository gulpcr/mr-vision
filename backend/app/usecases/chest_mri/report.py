"""Two-pass MedGemma orchestration for an MRI region plugin.

Identical in shape to the ``abdomen_ct`` report orchestration (the shared task hook
drives whichever plugin's ``report`` module is running). :func:`scan_and_report` runs the
whole flow against an already-constructed MedGemma client (dependency-injected by the
Celery task hook — this module imports no infrastructure and does no file I/O):

  Pass 1 (scan):   the per-slice scan images are chunked into batches of ``batch_size``
                   and sent to MedGemma one batch at a time, looping until EVERY slice
                   has been reviewed. Each reply flags the abnormal z-levels; they
                   accumulate across batches.
  Pass 2 (report): the flagged levels (capped to ``max_report_levels``, or a
                   representative fallback set when nothing was flagged) are sent back to
                   MedGemma for the structured findings/impression report.
...
async def rich_read(
    *,
    ...
    """Detailed-read pass: characterize findings + systematic organ/structure review.

    ``images`` is an ordered ``[{"z", "bytes"}]`` set of slices spanning the volume
    (flagged levels + coverage), superior→inferior. Returns
    ...
    prompt = prompts.build_richread_prompt(
        [int(e["z"]) for e in images], flagged or [], study_description, demographics, sequences
    )
    ...
    logger.info("mri_report_rich_read", images=len(images), has_report=bool(parsed))
    return parsed
...
async def scan_and_report(
    *,
    ...
    ``scan_images_by_window``: ``{window: [{"z", "name", "bytes"}]}`` — one image per
      candidate slice per SCAN window, superior→inferior (MRI uses a single window). Each
      window is scanned over the whole volume independently and the flagged levels are
      unioned.
    ``report_images_by_z``: ``{z: [{"name", "window", "bytes"}]}`` — the report-window
      images per level, used for the reporting pass.
    """
    ...
    for window in scan_windows:
        ...
        for i, batch in enumerate(batches, 1):
            prompt = prompts.build_scan_prompt(
                ...
                sequences=sequences, demographics=demographics,
            )
