...
Unlike ``ct_face`` — which samples a handful of axial levels (evenly or via a
VQVAE anomaly selector) and hands them straight to MedGemma — ``ct_face`` does
...
hook, using ``app.usecases.ct_face.report`` + ``prompts``). The rendered slices
...
Design choices mirror ct_face: fixed HU windowing (not percentile — it flattens
lesion-vs-parenchyma contrast), radiological display orientation, and NON-DIAGNOSTIC
assistive output. The trade-off vs ct_face is cost: scanning the whole volume is
...
class Pipeline(BasePipeline):
    ...
    def __init__(self) -> None:
        ...
        self._model_version = "ct_face_medgemma_v1.0.0"
    ...
    def preprocess(
        ...
        if primary is None:
            raise ValueError("No series found for ct_face pipeline")
    ...
    def infer(self, preprocessed: dict[str, Any], working_dir: str) -> dict[str, Any]:
        # The "inference" for ct_face is the MedGemma two-pass scan/report, which
        ...
        logger.info("ct_face_infer_passthrough")
    ...
    def postprocess(self, inference_output: dict[str, Any], working_dir: str) -> dict[str, Any]:
        logger.info("ct_face_postprocess_start")
        ...
        for z in preview_z:
            ...
            if entry:
                artifacts.append({
                    "name": entry["name"],
                    "artifact_type": "ct_face_slice_png",
    ...
    def _candidate_slices(self, arr: np.ndarray) -> list[int]:
        ...
        if len(kept) > cap:
            ...
            logger.info("ct_face_candidates_strided", available=len(kept), scanned=len(picked))
            return picked
        return kept
    ...
    def _render_png(self, slice_2d: np.ndarray, out_path: str, level: float, width: float) -> bool:
        ...
        try:
            ...
            logger.warning("ct_face_render_failed", error=str(exc))
            return False
