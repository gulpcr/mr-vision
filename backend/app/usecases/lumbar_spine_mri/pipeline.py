...
class Pipeline(BasePipeline):
    ...
    def postprocess(self, inference_output: dict[str, Any], working_dir: str) -> dict[str, Any]:
        ...
        # Volume for the optional geometric measurement pass: the SAGITTAL reference (best
        # plane for level naming / listhesis / disc height), else the primary plane's ref.
        measure_volume = None
        _mp = "sagittal" if "sagittal" in planes_info else (prim if prim in planes_info else None)
        if _mp:
            _rseq = planes_info[_mp].get("ref_seq")
            measure_volume = plane_volumes.get(_mp, {}).get(_rseq)
