...
REGION: dict[str, Any] = {   'exam_phrase': 'a LUMBAR SPINE MRI',
    'region_words': 'the lumbar spine',
    'richread_image_phrase': 'sagittal (and axial) multi-sequence MR panels of the lumbar spine',
    'targets': 'a disc herniation or bulge, spinal canal or neural-foraminal stenosis, '
               'spondylolisthesis or vertebral malalignment (including scoliosis / abnormal '
               'lordosis), cord or cauda equina compression, abnormal cord signal, vertebral '
               'collapse or a marrow signal abnormality (T1-dark / STIR-bright), or a '
               'paravertebral / epidural collection or mass',
    'scan_normal_clause': 'NORMAL vertebrae, NORMAL discs and a NORMAL thecal sac are NOT '
                          'findings, and bright-T2 CSF is normal. Do NOT flag a level merely '
                          'because the spine, discs or canal are visible.',
    'scan_finding_examples': '"disc herniation with canal narrowing", "L3 marrow lesion '
                             '(T1-dark/STIR-bright)", "epidural collection"',
    'organ_review': 'the vertebral bodies and marrow signal, the intervertebral discs, the spinal '
                    'canal and its calibre, the conus and cauda equina, the neural foramina, the '
                    'facet joints, and the paravertebral soft tissues',
    'relationship_examples': 'e.g. indenting the thecal sac, narrowing the canal or a neural '
                             'foramen, contacting or displacing a nerve root, extending into the '
                             'epidural space',
    'signal_patterns': 'In the spine: CSF is bright on T2; normal discs are bright on T2 and lose '
                       'signal with degeneration; vertebral marrow is bright on T1 (fatty) — a '
                       'marrow lesion is typically T1-dark and STIR-bright; STIR highlights '
                       'oedema.',
    'specificity_rules': 'SPECIFICITY: incidental degenerative change with NO neural compromise — '
                         'isolated disc desiccation, a mild disc bulge that does not narrow the '
                         'canal or a foramen, minor endplate or facet change — is expected with age; '
                         'describe it as degenerative and do not over-state it. BUT disc herniation, '
                         'spinal-canal or neural-foraminal stenosis, thecal-sac / nerve-root / cord '
                         'compression, and spondylolisthesis ARE clinically significant even when '
                         'degenerative in origin: report each with its level and GRADED severity '
                         '(mild / moderate / severe), and do NOT downgrade, dismiss, or omit them as '
                         'merely "age-related". Only call a marrow or cord signal abnormality when '
                         'corroborated (e.g. T1-dark AND STIR-bright); normal CSF is not a finding.',
    'age_sensitive': False,
    'plane_word': 'sagittal'}
...
def build_scan_prompt(
    ...
    # Cross-sequence corroboration only makes sense with >1 sequence. On a single-sequence
    # study (e.g. T2-only) requiring it would suppress EVERY flag, so it is dropped there.
    # Either way, morphological findings (bulge, herniation, stenosis, listhesis) never need
    # a second sequence — they are read from the plane that shows them.
    multi_seq = bool(sequences) and len(sequences) > 1
    corrob = (
        "For a MARROW or CORD signal abnormality, confirm it is corroborated across the "
        "sequences shown (e.g. a marrow lesion is T1-dark AND STIR-bright); a signal on only "
        "one sequence with no correlate is usually artefact. Morphological findings do NOT "
        "need cross-sequence corroboration.\n\n"
        if multi_seq else
        "This is a single-sequence study — judge each finding from the one sequence shown; do "
        "NOT withhold a morphological finding merely because a second sequence is unavailable.\n\n"
    )

    return f"""You are a board-certified radiologist performing a SENSITIVITY-FIRST triage of \
{REGION['exam_phrase']}.{context} No clinical history is assumed — assess each slice on its own merits.
...
This is a TRIAGE step whose goal is RECALL: catch every level a radiologist would comment on, and \
err toward flagging when a finding is plausible (a false flag is corrected in the reporting pass; a \
missed level is not). Flag a level when you can point to a SPECIFIC finding on it — {_TARGETS}. \
DEGENERATIVE findings ARE reportable here: flag disc bulges and herniations, disc desiccation with \
canal or lateral-recess narrowing, neural-foraminal stenosis, facet arthropathy causing stenosis, \
and any vertebral slip or malalignment — even when only mild — and grade severity (mild / moderate \
/ severe) in the finding text. {corrob}CRITICAL: {REGION['scan_normal_clause']} A genuinely normal \
level should still get an empty entry — but in a symptomatic screening population a completely \
normal lumbar spine is the exception, not the rule.

Rules:
- Judge only from what is visible across the tiles; do NOT invent measurements.
- In "finding", NAME the specific abnormality with its severity \
(e.g. {REGION['scan_finding_examples']}, "L4-L5 disc bulge with moderate canal stenosis", \
"grade-II L5-S1 spondylolisthesis"). If the structures are genuinely normal, that is NOT a \
finding — leave the level out.
- Refer to each level by the exact z index listed above ({zs}).
- Return an empty "anomalies" list only for a level that shows purely normal anatomy.
