...
Run INSIDE the worker container (it has monai==1.4.0):

    docker compose exec worker python backend/scripts/validate_abdomen_ct_anomaly.py
...
def main() -> int:
    ...
    sys.path.insert(0, str(USECASE.parents[2]))  # backend/ on path for `app.` import
    from app.usecases.abdomen_ct.anomaly import AnomalySliceSelector

    sel = AnomalySliceSelector(cfg, USECASE)
    try:
        # strict load check up front (AnomalySliceSelector uses default strict=True)
        sel.load()
    except Exception as exc:
        print(f"FAIL: model load: {exc}")
        return 1
    print(f"OK: loaded on {sel._device}, checksum {sel.checksum}")

    # Dummy volume: 40 axial slices of 256x256 HU, one 'lesion' blob.
    import numpy as np
